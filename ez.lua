...
local __BLACKSIGIL_TELEPORT_BOOT = __BLACKSIGIL_ENV.EXECO_TELEPORT_BOOT == true
__BLACKSIGIL_ENV.EXECO_TELEPORT_BOOT = nil
...
SavePersistentState = function()
    ...
    if type(units) == "table" then
        for unitID, unitData in pairs(units) do
            if type(unitData) == "table" and unitData.EXECOMock == true then
                mockUnits[unitID] = JsonSafeCopy(unitData)
            end
        end
    end
...
local function IsMockUnitID(unitID)
    ...
    if type(data) == "table" and data.EXECOMock == true then
        MockUnitIDs[unitID] = true
        return true
    end
...
local function DeleteAllMockData()
    ...
    if okRoot and type(currentRoot) == "table" then
        ...
        for unitID, unitData in pairs(newUnits) do
            if MockUnitIDs[unitID] or (type(unitData) == "table" and unitData.EXECOMock == true) then
                newUnits[unitID] = nil
            end
        end
...
local function QueueEXECOForTeleport()
    local queueFn =
        queue_on_teleport
...
featureSection:AddButton({
    ...
    Callback = function()
        ...
        local queued = QueueEXECOForTeleport()
        if not queued then
